...
1. **Select Media（选择媒体文件）**——浏览到一个或多个视频/音频文件，勾选每个想转录的文件；跨文件夹导航时已勾选的项不会丢失。文件列表上方的路径框也支持直接输入或粘贴路径（比如从资源管理器"复制为路径"得到的路径）——粘贴一个文件夹会直接跳转过去，粘贴一个文件则会跳转到它所在的文件夹。
