...
4. **[transcribe](transcribe/CLAUDE.md)** — the media→wav→srt pipeline pieces (ffmpeg invocation, the `Engine` interface, `WhisperCppEngine`, health checks, model scanning). Depends on `procutil`.
